...
program
  .usage('-n <name> -t <target> -g <git> -w <web> -b <branch> -d <dir> -u <user> -p <pwd>')
...
if (!(name && target && branch && web && dir && user && password && type)) {
  program.help();
}
...
mkdirp.sync(tmpdir);
const deploy = spawn('bash', [path.join(__dirname, '../sh/deploy.sh'), name, target, branch, web, dir, user, password, type]);
